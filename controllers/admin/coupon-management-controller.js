...
const getCouponPage = async (req, res, next) => {
  try {
    ...
    const couponUserCount = await Coupon.aggregate([
      { $group: { _id: null, usedCount: { $sum: '$usedCount' } } },
    ]);
    const totalUsedCount =
      couponUserCount.length > 0 ? couponUserCount[0].usedCount : 0;
...
const getAddCoupon = async (req, res, next) => {
  try {
    ...
    console.error('error while loading add coupon page', error.message);
    next(error);
  }
};
...
const addCoupons = async (req, res) => {
  try {
    ...
    if (!Number.isFinite(discount) || discount <= 0)
      return res.status(400).json({
        success: false,
        message: 'discountAmount must be a number greater than 0',
      });
    if (type === 'percentage' && discount > 100)
      return res.status(400).json({
        success: false,
        message:
          'For percentage type, discountAmount must be between 0 and 100',
      });
    if (!Number.isFinite(minPurchase) || minPurchase < 0)
      return res.status(400).json({
        success: false,
        message:
          'minimumOrderAmount must be a number greater than or equal to 0',
      });
    if (!start || isNaN(start.getTime()))
      return res
        .status(400)
        .json({ success: false, message: 'startDate is invalid or missing' });
    if (!expiry || isNaN(expiry.getTime()))
      return res.status(400).json({
        success: false,
        message: 'expirationDate is invalid or missing',
      });
    if (start > expiry)
      return res.status(400).json({
        success: false,
    ...
    if (expiry < today)
      return res.status(400).json({
        success: false,
        message: 'expirationDate must be today or a future date',
      });
    if (!Number.isInteger(userUsageLimit) || userUsageLimit < 1)
      return res.status(400).json({
        success: false,
        message: 'maxUsesPerUser must be an integer of at least 1',
      });
    if (
      !(usageLimit === 0 || (Number.isInteger(usageLimit) && usageLimit >= 1))
    )
      return res.status(400).json({
        success: false,
        message:
          'totalUsageLimit must be an integer of at least 1, or empty for unlimited',
      });
    ...
    if (type === 'flat') {
      ...
      if (!Number.isFinite(maxDiscount) || maxDiscount <= 0) {
        return res.status(400).json({
          success: false,
          message:
            'maxDiscount is required and must be > 0 for percentage discounts',
        });
      }
    }
...
const getEditCoupons = async (req, res, next) => {
  try {
    ...
    console.error('Error while loading get edit coupons ', error.message);
    next(error);
  }
};
...
const editCoupons = async (req, res) => {
  try {
    ...
    let usageLimit =
      bodyUsageLimit === null ||
      bodyUsageLimit === undefined ||
      bodyUsageLimit === ''
        ? 0
        : Number(bodyUsageLimit);
    ...
    if (type === 'percentage' && discountNum > 100)
      return res.status(400).json({
        success: false,
        message: 'For percentage type, discount must be between 0 and 100',
      });
    if (!Number.isFinite(minPurchaseNum) || minPurchaseNum < 0)
      return res
        .status(400)
        .json({ success: false, message: 'minPurchase must be a number >= 0' });
    if (!start || isNaN(start.getTime()))
      return res
        .status(400)
        .json({ success: false, message: 'startDate is invalid or missing' });
    if (!exp || isNaN(exp.getTime()))
      return res
        .status(400)
        .json({ success: false, message: 'expiry is invalid or missing' });
    if (start > exp)
      return res.status(400).json({
        success: false,
        message: 'expiry must be on or after startDate',
      });
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    if (exp < today)
      return res.status(400).json({
        success: false,
        message: 'expiry must be today or a future date',
      });
    if (!Number.isInteger(userLimitNum) || userLimitNum < 1)
      return res.status(400).json({
        success: false,
        message: 'userUsageLimit must be an integer of at least 1',
      });
    if (
      !(usageLimit === 0 || (Number.isInteger(usageLimit) && usageLimit >= 1))
    )
      return res.status(400).json({
        success: false,
        message:
          'usageLimit must be an integer of at least 1, or empty for unlimited',
      });
