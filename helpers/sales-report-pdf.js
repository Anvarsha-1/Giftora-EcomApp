...
function generateSalesReportPDF(orders, summary, res) {
  ...
  doc.text(
    `Net Revenue (Excl. Cancelled): ${formatCurrency(summary.netRevenue)}`,
  );
  ...
  doc.text(
    `Total Returns (Cancelled): ${formatCurrency(summary.totalReturns)}`,
    { align: 'right' },
  );
  doc.moveDown(2);
  doc
  ...
  const drawTableRow = (order) => {
    ...
    doc.text(formatCurrency(order.discount), 390, startY, {
      width: 60,
      align: 'right',
    });
    doc.font(FONT_BOLD).text(formatCurrency(order.netPaidAmount), 460, startY, {
      width: 70,
      align: 'right',
    });
  ...
  doc
    .font(FONT_BOLD)
    .fontSize(14)
    .text('Detailed Orders', { underline: true })
    .moveDown();
  drawTableHeader();
  ...
  doc
    .font(FONT_BOLD)
    .fontSize(12)
    .text('Final Report Summary', { underline: true })
    .moveDown(0.5);
  doc.font(FONT_REGULAR).fontSize(10);
  ...
  for (let i = range.start; i < range.count; i++) {
    ...
    doc
      .fillColor('#555')
      .text('GIFTORA Sales Report', 40, doc.page.height - 35, {
        align: 'left',
      });
    doc.text(`Page ${i + 1} of ${range.count}`, 40, doc.page.height - 35, {
      align: 'right',
    });
  }
