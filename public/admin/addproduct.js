 src="https://cdn.jsdelivr.net/npm/cropperjs@1.5.12/dist/cropper.min.js"
    ...
    const croppedImages = [null, null, null];
      ...
      function initializeCropper(imageElement, inputElement, index) {
        ...
        cropModal.style.left = sidebar.classList.contains("collapsed") || window.innerWidth <= 768 ? "70px" : "250px";
        cropModal.style.width = sidebar.classList.contains("collapsed") || window.innerWidth <= 768 ? "calc(100% - 70px)" : "calc(100% - 250px)";
        if (cropper) {
          cropper.destroy();
        }
        ...
        input.addEventListener("change", (e) => {
          ...
          if (file) {
            ...
            reader.onload = (e) => {
              ...
              document.querySelector(`.remove-image-btn[data-index="${index + 1}"]`).style.display = "block";
            };
      ...
      document.getElementById("cropButton").addEventListener("click", () => {
        if (cropper) {
            ...
            canvas.toBlob((blob) => {
              ...
              document.querySelector(`.remove-image-btn[data-index="${currentIndex + 1}"]`).style.display = "block";
              cropModal.style.display = "none";
              cropper.destroy();
              cropper = null;

              const dataTransfer = new DataTransfer();
              dataTransfer.items.add(new File([blob], `cropped-image${currentIndex + 1}.jpg`, { type: "image/jpeg" }));
              currentImageInput.files = dataTransfer.files;
              console.log(`Cropped image ${currentIndex + 1} saved to input`, currentImageInput.files);
            }, "image/jpeg", 0.8);
      ...
      document.getElementById("cancelCrop").addEventListener("click", () => {
        ...
        document.querySelector(`.remove-image-btn[data-index="${currentIndex + 1}"]`).style.display = "none";
